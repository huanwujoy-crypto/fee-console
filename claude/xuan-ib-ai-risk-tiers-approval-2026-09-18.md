...
它是**测量与披露**层面的修复：不新增 tier，不修改任何已批准系数，不扩大账户
范围，不引入任何交易权限，也不构成任何 AM／PM 运行成功的证据。
...
| IB-HK MSTR | AUTO T1 80% · $80,533 | 09-10 / 09-11 早间版：「不进分子（全部计入分母）：MSTR 判不适用」 | 登记表转录时只收录有系数的行，MSTR 无系数因而缺席。 |
...
2. **MSTR 转录为具名特例 `REG-SPECIAL-MSTR-NOT-APPLICABLE`**，kind
   `published-not-applicable`：不进分子、全部计入分母、无系数，披露原因
   `published-not-applicable`。这只是把 09-10／09-11 早间版已发布的处理方式搬进
   登记表；MSTR 是否应归 T1／T2／T3 仍是所有者的另一项决定，本文件不作。
...
5. **系数解析器与门禁**按同样的 instrumentId 优先规则解析 REG 记录；一条站在已登记
   标的上的 AUTO 记录（无论拼写、无论账户）被拒绝为 `AUTO_SHADOWS_REGISTRY_RULE`。
   门禁与计算模块接受登记表来源的排除原因。
...
| 修正后 | $1,486,975.46 | 22.97% |

覆盖变为 31 / 41：9 项非普通股排除、1 项（MSTR）按已发布口径排除；不再有 AUTO
分类记录。此回放不发布，不改写历史页面，不构成任何运行证据。
