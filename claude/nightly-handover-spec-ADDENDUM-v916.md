...
## 4. 概览不再显示夜间事件卡（Wu 已批，2026-09-16）

删除「今夜你睡着时会发生什么」及早间同族「接下来会发生什么」卡；需要睡前关注的挂单只在「待办 → 挂单提醒」显示。输入中的事件资料继续按现有 schema 验证并保留用于计算或审计，但不渲染为概览卡；不得为了填充概览而重复展示无操作价值的文字。

### 4.1 手机五栏导航固定单行（Wu 已批，2026-09-01 新增独立 ETF 子模块）

报告主体的五个导航按钮在 390px 手机宽度必须始终同排、不得换行。短标签和视觉顺序固定为「**概览 / 待办 / 配置 / 风险 / ETF**」；`ETF` 的无障碍名称为「XUAN-ETF 计划」。语义身份保持不变：概览 `s1/p1`、风险 `s2/p2`、配置 `s3/p3`、待办 `s4/p4`、ETF `s5/p5`，只交换风险与待办的可见按钮位置；pane DOM 仍固定为 `p1 / p2 / p3 / p4 / p5`。采用五等分布局；在 `<=360px` 时进一步缩小 gap、横向 padding 和字号。验收要求为：五键单行、水平溢出 0、最小可点击高度 44px。

固定入口不再显示可点击「刷新」按钮。入口在打开、回到前台及可见期间自动核对已发布版本；更新期间显示「更新中，请稍候」，成功后显示更新时间，失败时保留上一份已验证报告并自动重试。不得把多次点击当成生成或催促新报告的机制。
...
固定 loader 的稳态状态只显示「已同步 HH:MM」，不再重复报告日期、版别或 `L <build>`；已取消的临时报告不再保留生成中或完成提示。加载失败并回退到上一份已验证报告时，状态必须显示该旧报告的日期与版别，避免把陈旧内容误认成当前报告。内部 build 仍用于 cache busting 与排错，但不作为日常可见文案。

每个页签只保留一个不带 `open` 的 `<details>`，不得再套「来源与完整口径」等第二层说明：

> **报告说明 · 页签名**

展开体只保留一至三条真正影响本页理解的短句：

1. **概览**：日期/来源；仅在实际缺失时说明日涨跌不可用；
2. **风险**：压力情景不是收益预测；只追加本期真实近似或缺口；
3. **配置**：补仓是现金规划，具体金额必须留在正文；
4. **待办**：只列用户必须处理的事项；挂单仅提醒；
5. **ETF**：A/B/C 定义与同额出入金比较口径。

共同口径在全文档只保留一次，不复制到每个页签。账户关联披露保持唯一；原始来源节点可由显示层留作隐藏诊断记录，但不得形成可见的第二层折叠。已取消的临时报告、四桶配置与旧处理记录不出现在日常可见说明。不影响当期解读的规范版本号、详细入参、逐端点过程和重复免责措辞改由当晚存档与 GitHub 发布记录保留。页底只允许一行短语：「只读报告 · 数据截至 HH:MM HKT · 已验证发布 · 不是交易指令」。

若 IB / Sharesight / mapping 存在可接受的字段级回退，可见黄条压缩为一行，只写「受影响项目 + 替代数据日期 + 其余数据是否已更新」；必要的替代源、as-of 与影响范围在本页说明中直接列出，不再嵌套。关键数据无法核实、没有获批回退或发布验证失败时，必须显示不可折叠的红色 fail-closed 提示并保留上一份合格报告，不得以折叠隐藏。
