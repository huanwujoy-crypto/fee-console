...
在可机读的 holding / portfolio → 四桶映射尚未写入仓库、因而无法可靠拆分 Sharesight `Semi Liquid` 为「对冲基金」与「常青」时，**不得发明归类，也不得因此阻断整份报告**。仅四桶、三层流动性及直接依赖这些归桶的 KPI 沿用上一份已核实的周一全量快照，并在手机页与当晚存档显著标注「四桶归类沿用 YYYY-MM-DD，机器可读 mapping 待固化」。其余成功获取的 IB、Sharesight、公开市场及订单只读数据仍使用本次实时值；任一数据源失败仍按本节上一条逐项标注，不得填零或静默沿用。

该回退在仓库加入并通过测试的确定性四桶 mapping 后自动退役；届时恢复本节的每晚实时四桶计算。发布链路仅在候选、Validate、Promote、Pages 或线上读回失败时 fail-closed，不得把「mapping 尚未固化但存在已验证周一快照」单独作为整份报告停止发布的理由。
