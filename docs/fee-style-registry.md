...
4. 有缺口时，由 Claude 根据发行人业务/产品资料及已核对的持仓身份分类，再由独立
   ...
   首次记录必须 `effectiveFrom = firstHeldOn`，并由复核确认分类适用于该持仓期间；
   不得取“今天”作为更晚生效日，否则会妨碍周末回看。证据不支持全期时停止并处理
   受审的分期修订，不伪造首笔日期或用当前分类掩盖实际风格变化。
...
`firstHeldOn` 来自该 holding 的首笔交易日期；`effectiveFrom` 必须等于它，亦不能
...
- 静态已审映射优先，同账户/holding 的 ticker 冲突一律阻断；相同 ticker 的新
  ...
  全局静态 effectiveDate 固定为 2026-08-28，测试防止因上调而使历史映射集体失效。
...
独立复核已确认原运行环境可使用只读子代理完成第二上下文复核。合并后、首次启用前，
需实际修改原 Routine（保持时间、环境、账户范围及金融权限不变）：

- 第九节：用本文件的输入生成、缺口取证、独立复核、预检步骤替换手填风格总数。
- 第六节命令模板：删除 `--growth/--value`，设置 `FEE_STYLE_INPUT_FILE`。
- 第十节第 6/8 条：finally 清理私密风格输入；两遍幂等使用同一稳定输入。
- 第七节汇报：新增分类事件数量及实际私密投递审计；无证据不声称已通知。
- 第一节只允许修改 `data.json` 的限制保留。

以上是待发布后执行并回读验证的切换清单，不代表线上 Routine 已修改。
