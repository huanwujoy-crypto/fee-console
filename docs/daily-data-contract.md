...
- `--spy` / `--qqq` 是实际 `bd` 的**原始收盘价**（美股收盘），`--spyd` / `--qqqd`
  ...
  **不要传 Yahoo 的 `adjclose`**：它每逢除息回溯改写全部历史值，而这里每天只写一次
  ...
  `r = (P + (1 − w) × D) / P_prev − 1` 自己链式得出，其中 `w` 是页面用
  `BENCH_DIVIDEND_WITHHOLDING` 声明的美国股息预扣税率（非协定持有人 30%，2026-09-18
  业主确认）；台账与缓存始终只存毛股息 `D`，税率只在页面消费时应用，未声明即视为 0。
...
券商 1–3 日延迟、拆分小差和暂时无法判定的资金变动仍不阻断，只标暂估。合法的较早
benchmark date 也允许 AUM 点写入，但必须保留 `bd`，并由页面收紧 benchmark 截止日；
未来、过旧、回退、同一 `bd` 改价或 carry 日重复股息属于基准证据错误，只拒绝有问题的
benchmark bundle 并告警，不得阻断独立的 AUM 写入或费用回执。后续拿到可核实的同日价格后，
以 same-date replacement 补齐。
...
- `benchmark-cache` 的常规历史序列仍由 Yahoo daily chart 提供，并用 raw close、现金股息和
  adjusted-close 因子互相验证。若 Yahoo daily chart 或 completed-session meta 尚未前进，程序可
  使用 Nasdaq 官方 quote 与 historical 两个 completed-close 端点的相同交易日期和相同收盘价；
  Yahoo meta 已覆盖该日时再作为第三项交叉验证，仍落后时不阻断已由双端点确认的收盘价。SPY
  还必须处于 State Street 官方年度除息日历覆盖范围内，QQQ 必须成功读取 Nasdaq
...
- 自动缓存重试延长到香港时间 17:35；cloud producer 在 15:50 和 17:50 增加幂等补跑，使较晚
  到达的已验证行情可在同日生成 replacement，不依赖 Mac 开机。
