...
test('reads the exact owner custom group and excludes theme and defensive assets from the four-class denominator', () => {
  ...
  assert.equal(allocation.excludedValue, 150);
...
test('unknown, ungrouped, duplicate and misplaced USSC rows fail closed', () => {
  ...
  assert.throws(() => parseSharesightStockAllocation(withHolding(holding(8, '', 1, 'NEW'))), /UNGROUPED_HOLDING/);
  assert.throws(() => parseSharesightStockAllocation(withHolding(holding(8, '其它', 1, 'NEW'))), /UNKNOWN_ASSET_CLASS/);
