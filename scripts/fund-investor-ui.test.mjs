...
const fixedRemote=html.includes('fund-remote-storage:start');
...
const test=(name,fn,opt={})=>nodeTest(name,{skip:!active||!!opt.skip},fn);
...
const shellBuild=html.match(/<meta name="fee-console-build" content="([^"]+)">/)?.[1]||'current';
...
test('synthetic import stores only authenticated encrypted data and preserves source',async()=>{
 const c=context();await importProfile(c);const raw=run(c,'cfg(FUND_PROFILE_KEY)');assert(raw,run(c,'_fundProfileMessage'));assert(!raw.includes('SYNTHETIC'));assert(!raw.includes('ALPHA'));assert.equal(JSON.parse(raw).enc,true);const read=await run(c,'readFundProfile()');assert.equal(read.profile.fundName,'SYNTHETIC FUND');assert.equal(run(c,'cfg(K.gid)'),'synthetic-gist-A');assert(!run(c,'viewLink()').includes('synthetic-manager-token'));
});
test('legacy shared link remains readable during migration but new links stay fixed',async()=>{
 ...
 if(fixedRemote){assert(!run(reader,'viewLink()').includes('&fund='));assert(!run(reader,'viewLink()').includes('&archive='));assert(!run(reader,'viewLink()').includes('&docs='));}else assert(run(reader,'viewLink()').includes('&fund='));
});
...
test('configuration disappearance and restoration keeps the legacy local fallback without putting it in the URL',async()=>{
 const c=context();await importProfile(c);const before=run(c,'cfg(FUND_PROFILE_KEY)');run(c,"setCfg(K.gid,'')");const missing=await run(c,'readFundProfile()');assert.equal(missing.profile,null);run(c,"setCfg(K.gid,'synthetic-gist-A')");const restored=await run(c,'readFundProfile()');assert.equal(restored.profile.fundName,'SYNTHETIC FUND');assert.equal(run(c,'cfg(FUND_PROFILE_KEY)'),before);assert.equal(run(c,'viewLink()').includes('&fund='),!fixedRemote);
});
test('verified remote profile wins over a stale profile embedded in an old home-screen URL',async()=>{
 const c=context();await importProfile(c);const stale=run(c,'cfg(FUND_PROFILE_KEY)');run(c,`HASH.fund=${JSON.stringify(stale)};const fresh={...profile,fundName:'SYNTHETIC REMOTE LATEST'};_remoteFund={state:'verified',gid:cfg(K.gid),keyValue:cfg(K.key),revision:'remote-1',raw:'remote-cipher',bundle:{profile:fresh,library:{}} ,reason:''};`);const loaded=await run(c,'readFundProfile()');assert.equal(loaded.profile.fundName,'SYNTHETIC REMOTE LATEST');assert(!run(c,'viewLink()').includes('&fund='));
},{skip:!fixedRemote});
