...
// The calendar date in a named zone. Daylight saving is resolved by the zone,
// never by an offset the caller supplies.
export function zoneDate(zone, now = new Date()) {
...
// A weekend plus one market holiday is the longest legitimate gap between two
// common closes. Beyond that the comparison is not merely between sessions: it
// has stopped being updated, and its balances cannot be read as current ones.
export const ETF_STALE_AFTER_DAYS = 5;

// How far the last complete comparison date is behind the date it is being read
// on. The caller supplies that viewing date explicitly; this module never
// assumes the reader's clock is the report's date.
export function etfTrendAgeDays(data, viewDate) {
  check(validDate(viewDate), 'Invalid view date');
  check(viewDate >= data.latestCompleteDate, 'View date precedes the comparison data');
  return Math.round((Date.parse(`${viewDate}T00:00:00Z`) - Date.parse(`${data.latestCompleteDate}T00:00:00Z`)) / 86400000);
}

export function validateOpenEtfTrend(data, {now = new Date(), maxSeenDate = null} = {}) {
  validateTrend(data, true);
  const day = zone => zoneDate(zone, now);
...
export function renderEtfTrend(data, { privateResult = null, viewDate = null } = {}) {
  ...
  // Opt-in: a caller that states when the card is being read gets the age
  // disclosure. A frozen historical artifact passes nothing and is unchanged.
  const ageDays = viewDate === null ? null : etfTrendAgeDays(data, viewDate);
  const rows = data.rows, last = rows.at(-1), full = rows.find(r => r.date === data.latestCompleteDate);
  const values = rows.flatMap(r => Object.values(r.index));
  const lo = Math.min(100, ...values) - .25, hi = Math.max(100, ...values) + .25;
